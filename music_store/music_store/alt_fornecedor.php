...
if (!empty($_POST)) {
  ...
  $erro = 0;
  $query1 = "UPDATE fornecedor SET nome_fant='$nome', cnpj='$cnpj', inscricao_est='$ie' WHERE cod_forn='$cod'";


   $query2 = "UPDATE endereco SET logradouro='$lg', numero='$nm', cep=$cep, complemento='$cpm' WHERE cod_fornecedor='$cod'";


   $query3= "UPDATE telefone SET telefone='$tf',celular='$cl' WHERE cod_fornecedor='$cod'";
...
<?php
  ...
  include "conecta.inc";
  $cod = $_GET["cod"];
  $consulta = " SELECT * FROM fornecedor WHERE cod_forn = '$cod'";
  $consulta2 = "SELECT * FROM telefone WHERE cod_fornecedor = '$cod'";
  $consulta3 = "SELECT * FROM endereco WHERE cod_fornecedor = '$cod'";
...
<form id="cont" class="rounded" method="post" action="alt_fornecedor.php" name="form">
			<h2>Cadastro de Fornecedor</h3>
